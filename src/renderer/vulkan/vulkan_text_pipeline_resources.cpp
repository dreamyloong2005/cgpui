...
VkPushConstantRange vulkan_text_push_constant_range() {
  return VkPushConstantRange{
      .stageFlags = VK_SHADER_STAGE_VERTEX_BIT,
      .offset = 0,
      .size = sizeof(VulkanTextPushConstants),
  };
}

VkPipelineLayoutCreateInfo vulkan_text_pipeline_layout_create_info(
...
Result<void> vulkan_create_text_pipeline_resources(
  ...
  if (device == VK_NULL_HANDLE || render_pass == VK_NULL_HANDLE ||
      glyph_atlas_descriptor_set_layout == VK_NULL_HANDLE) {
    return std::unexpected(vulkan_error(
        ErrorCode::renderer_initialization_failed,
        "text pipeline requires device, render pass, and atlas layout"));
  }
  vulkan_destroy_text_pipeline_resources(device, resources);

  const VkPushConstantRange push_constant_range =
      vulkan_text_push_constant_range();
  const VkPipelineLayoutCreateInfo layout_info =
      vulkan_text_pipeline_layout_create_info(
          glyph_atlas_descriptor_set_layout,
          push_constant_range);
  if (auto result = require_vk_success(
          vkCreatePipelineLayout(
              device, &layout_info, nullptr, &resources.layout),
          "vkCreatePipelineLayout for text pipeline failed");
      !result) {
    return result;
  }

  VulkanTextShaderModules shader_modules;
  if (auto result =
          vulkan_create_text_shader_modules(device, shader_modules);
      !result) {
    vulkan_destroy_text_pipeline_resources(device, resources);
    return result;
  }
  ...
  const auto shader_stages =
      vulkan_text_shader_stage_create_infos(shader_modules);
  ...
  const auto dynamic_states = vulkan_text_pipeline_dynamic_states();
  ...
  const auto pipeline_result = require_vk_success(
      vkCreateGraphicsPipelines(
          ...
          nullptr,
          &resources.pipeline),
      "vkCreateGraphicsPipelines for text pipeline failed");
  vulkan_destroy_text_shader_modules(device, shader_modules);
  if (!pipeline_result) {
    vulkan_destroy_text_pipeline_resources(device, resources);
