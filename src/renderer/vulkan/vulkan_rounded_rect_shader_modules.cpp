...
Result<VkShaderModule> create_shader_module(
  ...
  const VkShaderModuleCreateInfo create_info =
      vulkan_rounded_rect_shader_module_create_info(spirv);
...
Result<void> vulkan_create_rounded_rect_shader_modules(
  ...
  if (device == VK_NULL_HANDLE) {
    return std::unexpected(vulkan_error(
        ...
        "rounded rectangle shaders require a Vulkan device"));
  }
  vulkan_destroy_rounded_rect_shader_modules(device, modules);
  auto vertex = create_shader_module(
      device,
      vulkan_rounded_rect_vertex_shader_spirv(),
      "vkCreateShaderModule for rounded rectangle vertex shader failed");
  if (!vertex) {
  ...
  auto fragment = create_shader_module(
      ...
      "vkCreateShaderModule for rounded rectangle fragment shader failed");
  if (!fragment) {
    vulkan_destroy_rounded_rect_shader_modules(device, modules);
