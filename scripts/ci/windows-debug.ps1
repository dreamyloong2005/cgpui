...
if ($env:OS -ne "Windows_NT") {
  throw "windows-debug.ps1 requires Windows"
}
...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
  $OutputRoot = Join-Path $repoRoot "build\phase-g-ci\windows-debug"
...
New-Item -ItemType Directory -Force -Path @(
  $buildRoot, $configRoot, $packageCache, $packageInstall,
...
try {
  & xmake f -P $repoRoot -y -c -m debug --ccache=n -o $buildRoot
...
$binaryRoot = Join-Path $buildRoot "windows\x64\debug"
$libraries = @(
  "cgpui_core.lib",
  "cgpui_platform.lib",
...
foreach ($library in $libraries) {
  ...
  if (-not (Test-Path -LiteralPath $source -PathType Leaf)) {
    throw "missing Windows Debug library: $source"
  }
...
if (-not (Test-Path -LiteralPath $demo -PathType Leaf)) {
  throw "missing Windows Debug demo: $demo"
}
...
if ($parsedManifest.schema_version -ne 1 -or
    $parsedManifest.package -ne "cgpui-windows-debug" -or
    @($parsedManifest.libraries).Count -ne $libraries.Count) {
  throw "Windows Debug manifest validation failed"
}
