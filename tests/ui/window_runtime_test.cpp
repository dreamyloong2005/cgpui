...
int main() {
  ...
  if (const int result =
          test_runtime_sends_accessibility_live_update_events_to_platform_window();
      result != 0) {
    return result;
  }
  ...
  if (const int result = test_event_route_carries_element_ancestry();
      result != 0) {
    return result;
  }
  if (const int result = test_event_route_carries_child_view_ancestry();
      result != 0) {
    return result;
  }
