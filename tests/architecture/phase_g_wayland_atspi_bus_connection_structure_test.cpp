...
int main() {
  ...
  const std::string* required[]{
      &bus_header, &bus_source, &adapter_header, &adapter_source, &adapter_bus,
      &services, &window, &behavior, &platform_inventory, &wayland_inventory,
  ...
  if (!contains(bus_source, "diagnostics_.discovery_failure_count += 1") ||
      ...
      !contains(bus_source, "diagnostics_.disconnect_count += 1")) {
    return 4;
  }
  if (!contains(adapter_header, "WaylandAtspiBusConnection bus_connection_") ||
      !contains(adapter_header, "automatic_connection_attempted_") ||
      !contains(adapter_bus, "ensure_dbus_connection(") ||
      !contains(adapter_bus, "if (automatic_connection_attempted_) return false") ||
  ...
  if (!contains(behavior, "org.a11y.Bus") ||
      !contains(behavior, "unix:path=/tmp/cgpui-atspi") ||
  ...
  if (line_count(bus_header) > 70 || line_count(bus_source) > 180 ||
      ...
      line_count(window) > 180 || line_count(behavior) > 190) {
    return 10;
  }
  ...
  if (!contains(
          ledger_json,
          "\"phase_f_current_handoff\": \"Step 633 Linux AT-SPI disconnect "
          "and reconnect production behavior\"")) {
    return 12;
  }
  return 0;
}
