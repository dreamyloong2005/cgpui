...
int main() {
  ...
  if (!contains(previous, "Step 672 reproducible dependency setup") ||
  ...
  if (!contains(vocabulary, "`TestPlatformServiceSnapshot`") ||
      ...
      !contains(ledger_json, "\"phase_f_current_handoff\": \"Step 672 reproducible dependency setup\"")) return 9;
  return 0;
}
