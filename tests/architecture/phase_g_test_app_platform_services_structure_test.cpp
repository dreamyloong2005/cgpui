...
int main() {
  ...
  if (!contains(previous, "Step 673 Windows full-debug verification") ||
  ...
  if (!contains(vocabulary, "`TestPlatformServiceSnapshot`") ||
      ...
      !contains(ledger_json, "\"phase_f_current_handoff\": \"Step 673 Windows full-debug verification\"")) return 9;
  return 0;
}
