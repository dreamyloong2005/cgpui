...
int main() {
  ...
  if (!contains(pointer_guard, "runtime_window_input_state.cpp") ||
      ...
      !contains(child_guard, "Step 636 async task priority production behavior")) return 4;
  ...
  if (!contains(ledger_json,
                "\"phase_f_current_handoff\": \"Step 636 async task priority production behavior\"")) return 7;
  return 0;
}
