...
struct WaylandTestCompositor::State {
  struct SurfaceState;
  using ClientDataSource = WaylandTestClipboardClientSource;
  ...
  void request_resize_configure_state(
      ...
      bool fullscreen) {
    configure.request(
        width, height, activated, maximized, fullscreen, true);
  }
  ...
  void request_toplevel_configure_state(
      ...
      bool fullscreen) {
    configure.request(
        width, height, activated, maximized, fullscreen, false);
  }

  void request_surface_configure() { configure.request_surface(); }
  ...
  clipboard_client_selection_mime_types() const {
    return clipboard_source.active_mime_types();
  }

  [[nodiscard]] std::string last_clipboard_client_selection_payload() const {
    return clipboard_source.last_payload();
  }
...
void WaylandTestCompositor::State::ack_configure(
  ...
  state->compositor->configure.acknowledge(serial);
}
...
void WaylandTestCompositor::State::data_device_manager_create_data_source(
  ...
  auto* source_state = new ClientDataSource{
      .compositor = static_cast<void*>(compositor),
  };
...
void WaylandTestCompositor::State::data_source_offer(
  ...
  auto* compositor = static_cast<State*>(source->compositor);
  compositor->clipboard_source.offer(source, mime_type);
}
...
void WaylandTestCompositor::State::data_device_set_selection(
  ...
  compositor->clipboard_source.set_selection(source, serial);
}
...
void WaylandTestCompositor::State::handle_client_data_source_destroyed(
  ...
  if (source->compositor != nullptr) {
    auto* compositor = static_cast<State*>(source->compositor);
    compositor->clipboard_source.destroyed(source);
  }
  delete source;
}
...
void WaylandTestCompositor::State::dispatch_pending_resize_configure() {
  const bool send_toplevel = configure.take_toplevel_pending();
  const bool send_surface = configure.take_surface_pending() ||
      (send_toplevel && configure.include_surface());
  if (!send_toplevel && !send_surface) return;
  ...
  for (const auto& surface : surfaces) {
    ...
    const WaylandConfigureState requested = configure.snapshot();
    if (send_toplevel) {
      wl_array states{};
      ...
      if (requested.maximized) add_state(xdg_toplevel_state_maximized);
      if (requested.fullscreen) add_state(xdg_toplevel_state_fullscreen);
      if (requested.activated) add_state(xdg_toplevel_state_activated);
      wl_resource_post_event(
          surface->toplevel,
      ...
      configure.mark_toplevel_sent();
    }
    if (send_surface) {
      const std::uint32_t serial = next_configure_serial++;
      configure.mark_surface_sent(serial);
      wl_resource_post_event(
          surface->xdg_surface, xdg_surface_configure, serial);
    }
    wl_display_flush_clients(display);
    return;
  }

  if (send_toplevel) configure.requeue_toplevel();
  if (send_surface) configure.requeue_surface();
}
...
void WaylandTestCompositor::request_toplevel_configure_state(
    std::int32_t width,
    std::int32_t height,
    bool activated,
    bool maximized,
    bool fullscreen) {
  state_->request_toplevel_configure_state(
      width, height, activated, maximized, fullscreen);
}

void WaylandTestCompositor::request_surface_configure() {
  state_->request_surface_configure();
}
...
bool WaylandTestCompositor::wait_for_resize_configure_sent() const {
  return state_->configure.wait_for_surface_sent();
}

bool WaylandTestCompositor::wait_for_resize_configure_acked() const {
  return state_->configure.wait_for_ack();
}

bool WaylandTestCompositor::wait_for_toplevel_configure_sent_count(
    std::uint32_t count) const {
  return state_->configure.wait_for_toplevel_sent_count(count);
}
...
bool WaylandTestCompositor::wait_for_clipboard_client_selection_set() const {
  return state_->clipboard_source.wait_for_selection_set();
}

bool WaylandTestCompositor::wait_for_clipboard_client_selection_set_count(
    std::uint32_t count) const {
  return state_->clipboard_source.wait_for_set_count(count);
}

bool WaylandTestCompositor::
    clipboard_client_selection_replacement_was_continuous() const {
  return state_->clipboard_source.replacement_was_continuous();
}
