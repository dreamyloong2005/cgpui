...
[[nodiscard]] inline TextWrapLine text_wrap_line_for_range(
...
[[nodiscard]] inline TextWrapLayout wrap_text_measurement(
...
[[nodiscard]] inline TextHitTestResult hit_test_text_position(
    const TextMeasurement& measurement,
    Rect bounds,
    Point point) {
  const TextShapeRun& run = measurement.shape_run;
...
[[nodiscard]] inline TextSelectionRange text_selection_range_from_points(
    const TextMeasurement& measurement,
    Rect bounds,
    Point anchor,
    Point head) {
