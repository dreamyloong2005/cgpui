...
- Created `.worktrees/phase-c-div-sizing-color-border-helpers` on
  `codex/phase-c-div-sizing-color-border-helpers` from
  `c1225c2 docs: mark phase c step 320 merged`.
- Baseline focused Windows gate passed:
  `xmake test -y -P . element_test/default style_test/default
  ui_source_structure_test/default gpui_parity_ledger_test/default
  public_phase_b_completion_audit_test/default` passed 5/5.
- RED added
  ...
  `xmake test -y -P . element_test/default` failed as expected because
  `ElementBuilder` did not have `w(...)`.
- RED extended the structure guard in `ui_source_structure_test` requiring
  `w(...)`, `h(...)`, and `border_1()` in
  `src/ui/element_builder_layout.cpp`, plus `bg(...)`, `text_color(...)`, and
  `rounded(...)` in `src/ui/element_builder_style.cpp`; `xmake test -y -P .
  ui_source_structure_test/default` failed as expected.
- GREEN added `w(...)`, `h(...)`, `bg(...)`, `text_color(...)`,
  `border_1()`, and `rounded(...)` as aliases over the existing
  size/background/foreground/border/radius style setters without adding layout
  engine, widget, or uniform-list behavior.
- Focused Windows GREEN verification passed:
  `xmake test -y -P . element_test/default` passed 1/1, and
  `xmake test -y -P . ui_source_structure_test/default` passed 1/1.
- Updated the Markdown/JSON parity ledger, the ledger guard, the complete
  replication roadmap, and working notes with Step 321 evidence. Step 322 is
  the next focused Phase C `div` style-vocabulary slice after Step 321 lands
  on `master`.
- Focused feature-worktree verification passed:
  `python -m json.tool docs\gpui-complete-parity-ledger.json` exited 0, and
  `xmake test -y -P . element_test/default ui_source_structure_test/default
...
- Fresh feature-worktree full verification passed:
  ...
  warnings, Windows `xmake test -P .` passed 99/99, and WSL Arch Linux
  `XMAKE_ROOT=y xmake test -P .` passed 96/96.
- Step 321 is implemented and Windows/WSL full-debug verified in the feature
  worktree. It is ready for feature commit and merge verification. Step 322 is
  the next Phase C `div` style-vocabulary slice after Step 321 lands on
  `master`.
- After adding the final full-verification evidence to the planning notes,
...
- Continued `.worktrees/phase-c-div-shadow-overflow-text-helpers` on
  `codex/phase-c-div-shadow-overflow-text-helpers` from
  `a63ab4f docs: mark phase c step 321 merged`.
- Existing RED coverage in `tests/ui/element_test.cpp` requires
  `overflow_hidden()`, `overflow_visible()`, `opacity(...)`, `z_index(...)`,
  `relative()`, and individual inset-edge helpers to map to existing `Style`
  fields; existing RED structure coverage in
  `tests/architecture/ui_source_structure_test.cpp` requires layout helpers in
  `src/ui/element_builder_layout.cpp` and style helpers in
  `src/ui/element_builder_style.cpp`.
- GREEN added declarations in `include/cgpui/ui/element_builder_core.hpp`,
  style helper bodies in `src/ui/element_builder_style.cpp`, and layout helper
  bodies in `src/ui/element_builder_layout.cpp`. The implementation stays an
  alias layer over existing `Style` fields and does not add widget,
  uniform-list, shadow-storage, text-style, or layout-engine behavior.
- Focused Windows GREEN verification passed:
  `xmake test -y -P . element_test/default ui_source_structure_test/default`
  passed 2/2.
- Updated the Markdown/JSON parity ledger, the ledger guard, the complete
  replication roadmap, and working notes with Step 322 vocabulary and module
  ownership. Step 323 is the next focused Phase C `div` style-vocabulary
  slice after Step 322 lands on `master`.
- The first six-test focused gate rerun failed only in
  ...
  exact guarded phrase `exclusions out` across a line break. Restored the
  phrase on one line, then reran the audit test successfully.
- Focused feature-worktree verification passed:
  `xmake test -y -P . element_test/default ui_source_structure_test/default
  gpui_parity_ledger_test/default public_authoring_vocabulary_freeze_test/default
  public_phase_b_completion_audit_test/default style_test/default` passed 6/6.
- Fresh feature-worktree full verification passed:
  ...
  warnings, Windows `xmake test -P .` passed 99/99, and WSL Arch Linux
  `XMAKE_ROOT=y xmake test -P .` passed 96/96.
- Step 322 is implemented and Windows/WSL full-debug verified in the feature
...
## 2026-07-07 Phase C Step 333 Merge

- Committed `codex/phase-c-theme-token-fallback` as
  `e1d5467 feat: add theme token fallback`.
- Fast-forward merged the branch into `master` at `e1d5467`.
- Verified post-merge Windows:
  `python -m json.tool docs\gpui-complete-parity-ledger.json` exited 0,
  `xmake f -c -m debug -P .` exited 0, and `xmake test -P .` passed 99/99.
- Verified post-merge WSL Arch Linux:
  `XMAKE_ROOT=y xmake f -y -c -m debug -P .` exited 0 and
  `XMAKE_ROOT=y xmake test -P .` passed 96/96.
- Verified closeout docs gates after the merge notes:
  Windows `xmake test -y -P . gpui_parity_ledger_test/default
  public_phase_b_completion_audit_test/default` passed 2/2, and WSL Arch
  Linux `XMAKE_ROOT=y xmake test -y -P .
...
- Committed `codex/phase-c-tab-focus-metadata` as
  `5290912 feat: add focus traversal metadata`.
- Fast-forward merged the branch into `master` at `5290912`.
- Verified post-merge JSON:
  `python -m json.tool docs\gpui-complete-parity-ledger.json` exited 0.
- Verified post-merge Windows:
  `xmake f -c -m debug -P .` exited 0 and `xmake test -P .` passed 100/100.
...
- Committed `codex/phase-c-click-drag-gestures` as
  `66125aa feat: synthesize click drag gestures`.
- Fast-forward merged the branch into `master` at `66125aa`.
- Verified post-merge JSON:
  `python -m json.tool docs\gpui-complete-parity-ledger.json` exited 0.
- Verified post-merge Windows:
  `xmake f -c -m debug -P .` exited 0 and `xmake test -P .` passed 100/100.
...
- Fast-forward merged `codex/phase-c-icon-image-widgets` into root `master` at
  `c4c0c85 feat: add image icon widgets`; the feature worktree had no
  remaining uncommitted diff before merge, and root `master` still only had
  the existing untracked `.vscode/`.
- Verified post-merge JSON:
  `python -m json.tool docs\gpui-complete-parity-ledger.json` exited 0.
- Verified post-merge diff hygiene:
...
- Verified post-merge WSL Arch Linux:
  `XMAKE_ROOT=y xmake f -y -c -m debug -P .` exited 0 and
  `XMAKE_ROOT=y xmake test -P .` passed 100/100.
- Step 347 is complete on `master`; Step 348 container primitive follow-up is
  next, but paused here per user request.

## 2026-07-08 Phase C Step 346 Merge

- Fast-forward merged `codex/phase-c-list-menu-widgets` into root `master` at
  `08a2547 feat: add list menu item widgets`; the feature worktree had no
  remaining uncommitted diff before merge, and root `master` still only had
  the existing untracked `.vscode/`.
- Verified post-merge JSON:
  `python -m json.tool docs\gpui-complete-parity-ledger.json` exited 0.
...
## 2026-07-08 Phase C Step 348 Merge

- Fast-forward merged `codex/phase-c-container-primitives` into root `master`
  at `780c31c feat: add container primitive builder`; root `master` still had
...
## 2026-07-08 Phase C Step 352 Merge

- Fast-forward merged `codex/phase-c-large-list-recycling` into root
  `master` at `90df746 feat: add uniform list recycling window`; root
  `master` still had no tracked diff and only the existing untracked
  `.vscode/`.
- Verified post-merge JSON:
  `python -m json.tool docs\gpui-complete-parity-ledger.json` exited 0.
...
## 2026-07-08 Phase C Step 363 Merge

- Fast-forward merged `codex/phase-c-svg-image-public-examples` into root
  `master` at `017bef5 feat: add svg image source public example`; root
  `master` still had no tracked diff and only the existing untracked
  `.vscode/`.
- Verified post-merge JSON:
  `node -e "JSON.parse(require('fs').readFileSync('docs/gpui-complete-parity-ledger.json','utf8')); console.log('json ok')"`
...
- Verified post-merge WSL Arch Linux with D-drive build/cache output and
  ...
  `TMPDIR=/dev/shm/cgpui XMAKE_ROOT=y XMAKE_GLOBALDIR=/mnt/d/Dev/Projects/cgpui/.build-wsl/master/global XMAKE_TMPDIR=/dev/shm/cgpui XMAKE_PKG_CACHEDIR=/mnt/d/Dev/Projects/cgpui/.build-wsl/master/pkg-cache XMAKE_PKG_INSTALLDIR=/mnt/d/Dev/Projects/cgpui/.build-wsl/master/pkg-install xmake f -P . -y -c -m debug --ccache=n -o /mnt/d/Dev/Projects/cgpui/.build-wsl/master/build-root`
  exited 0, and the matching
  `xmake test -j 1 -w /mnt/d/Dev/Projects/cgpui -P .` passed 110/110.
- Disk placement check after post-merge WSL verification: `.build-wsl/master`
  on D: was 5.2G, `/tmp` was 0, and `/root/.xmake` was absent. Step 364 should
  keep reusing the D-drive `.build-wsl/master` global/package caches and write
...
- Started from clean `master` after
  `7f20bbd feat: add text shaping metadata`; `git status --short --branch`
  showed only the existing untracked `.vscode/`.
- Added RED structure coverage in
  `tests/architecture/ui_source_structure_test.cpp` requiring
  `src/ui/text_shaping_harfbuzz.cpp`, an internal
  `shape_text_with_harfbuzz(...)` declaration, dispatch branching for
  `TextShapingBackend::harfbuzz`, and xmake renderer ownership. RED failed as
  expected with `ui_source_structure_test/default`.
- GREEN implementation added `src/ui/text_shaping_harfbuzz.cpp`, routed the
  selected HarfBuzz backend through that file, registered it with xmake, and
  kept disabled builds on deterministic fallback with
...
- Started from clean `master` after
  `6e48f70 feat: add paragraph layout cache`; `git status --short --branch`
  showed only the existing untracked `.vscode/`.
- Added RED behavior coverage in `tests/ui/text_model_test.cpp` requiring
  `\r\n` to form one measured grapheme column, split as a hard wrap, omit both
  CRLF glyphs from wrapped paint metadata, and keep the following line starting
  after the LF byte/glyph.
- Added RED structure coverage in
  `tests/architecture/ui_source_structure_test.cpp` requiring the focused
  `text_measurement_is_crlf_pair(...)` helper in
  `src/ui/text_measurement_grapheme.cpp` and CR recognition in
  `text_wrap_column_is_hard_break(...)`.
- RED failed as expected:
  `xmake test -y -P . text_model_test/default
  ui_source_structure_test/default` failed 2/2.
- GREEN implementation keeps CRLF together as one grapheme column, treats
  both CR and LF columns as hard-wrap breaks, and lets existing line/glyph
  ranges skip CRLF paint without adding hidden paragraph state.
- Focused GREEN verification passed:
  `xmake test -y -P . text_model_test/default
  ui_source_structure_test/default` 2/2.
- Adjacent Windows verification passed:
...
- Started from clean `master` after
  `873bf19 feat: add runtime double click word selection`; `git status
  --short --branch` showed only the existing untracked `.vscode/`.
- Added RED behavior coverage in `tests/ui/window_runtime_text_test.cpp`
  requiring a text input triple-click (`PointerButton::click_count = 3`) to
  select the current line on pointer down and keep that line selected on
  pointer up.
- Added RED structure coverage in
  `tests/architecture/ui_source_structure_test.cpp` requiring
  `src/ui/runtime_text_pointer_selection.cpp` to consume
  `TextModel::line_selection_range_at(...)` inside the focused runtime pointer
  selection boundary.
- RED failed as expected:
  `xmake test -y -P . window_runtime_text_test/default
  ui_source_structure_test/default` failed 2/2 before line granularity handling
  was implemented.
- GREEN implementation adds the `TextSelectionGranularity::line` branch beside
  the existing word branch, sets the model line range directly, and leaves
  single-click caret/drag selection unchanged.
- Focused GREEN verification passed:
  `xmake test -y -P . window_runtime_text_test/default
  ui_source_structure_test/default` 2/2.
- Adjacent Windows verification passed:
