...
| gpui animation | deterministic animation clock/tween, typed scalar transitions, runtime-owned keyed element lifecycle animations, infinite repeat stages, and indexed one-shot chains | Required | spring variants, cancellation diagnostics, frame pacing, full style interpolation, and official examples remain | Phase G |
