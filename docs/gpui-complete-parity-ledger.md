...
| gpui image assets | image paint/upload pipeline, bounded file/image/GIF/SVG decode paths, stable cache keys, thread-safe single-asset/source-wide revision invalidation, and bounded priority-aware async loading | Required | Final image/GIF examples remain | Phase C/G |
