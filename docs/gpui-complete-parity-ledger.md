...
| gpui image assets | image paint/upload pipeline, public source/registry authoring, and root-confined `AssetSource`/`FileAssetSource` binary loading with stable listing and byte limits | Required | PNG/JPEG/GIF decode, reload/cache invalidation, async loading, and final image/GIF examples remain | Phase C/G |
