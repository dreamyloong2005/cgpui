...
| gpui text system | `TextModel`, shaping, wrap, glyph records | Required | deterministic text depth exists; Phase D Steps 379-387 add `TextShapingBackend` selection, HarfBuzz capability reporting, explicit fallback reason metadata, glyph ids, glyph positioning offsets, shaping direction/script/language metadata, internal shaping dispatch/fallback backend boundaries, a guarded `src/ui/text_shaping_harfbuzz.cpp` insertion point, backend capability snapshots on selections/runs, wrapped glyph-id propagation, a text-shaping readiness audit, and a focused `src/ui/text_font.cpp` implementation boundary for font database/discovery helpers; production HarfBuzz shaping and native font discovery remain incomplete | Phase D |
