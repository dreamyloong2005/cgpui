...
| gpui image assets | image paint/upload pipeline, public source/registry authoring, root-confined binary loading, bounded PNG/JPEG/GIF decoding, and bounded SVG asset parsing plus raster bridging | Required | Cache keys, reload invalidation, async loading, and final image/GIF examples remain | Phase C/G |
