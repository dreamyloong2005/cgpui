...
| gpui::AsyncApp | bounded prioritized async task scheduling plus move-only parent/child `TaskGroup` ownership, recursive observability, descendant cancellation propagation, and foreground/background task Result conventions on `WindowRuntime`, `WindowRuntimeContext`, and `AsyncContextCapability` | Required | `include/cgpui/ui/task_group.hpp`, `src/ui/runtime_task_pool.cpp`, `src/ui/runtime_task_priority.cpp`, `src/ui/runtime_task_group.cpp`, `src/ui/runtime_task_group_propagation.cpp`, `tests/async/runtime_task_cancellation_propagation_test.cpp`; async I/O and full async app context remain | Phase G |
