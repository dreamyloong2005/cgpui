...
| gpui animation | deterministic animation clock/tween, typed scalar transitions, and runtime-owned keyed element lifecycle animations with scoped mount/update/unmount state and shared delayed frame wakeups | Required | repeat/chaining, spring variants, cancellation diagnostics, frame pacing, full style interpolation, and official examples remain | Phase G |
