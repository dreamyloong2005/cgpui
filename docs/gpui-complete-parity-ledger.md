...
| gpui text system | `TextModel`, shaping, wrap, glyph records | Required | deterministic text depth exists; Phase D Steps 379-380 add `TextShapingBackend` selection, HarfBuzz capability reporting, explicit fallback reason metadata, glyph ids, and internal shaping dispatch/fallback backend boundaries; production HarfBuzz shaping remains incomplete | Phase D |
