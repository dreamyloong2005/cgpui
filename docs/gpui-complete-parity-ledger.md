...
| gpui image assets | image paint/upload pipeline, public source/registry authoring, root-confined `AssetSource`/`FileAssetSource` binary loading, and bounded signature-detected PNG/JPEG RGBA8 decoding | Required | GIF decode, reload/cache invalidation, async loading, and final image/GIF examples remain | Phase C/G |
