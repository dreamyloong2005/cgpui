...
| gpui::actions! / action macro | `Action<T>` typed action concept, `action_name<T>()` / `action_name_v<T>`, and typed register/dispatch overloads over the existing string action registry | Required | `include/cgpui/ui/action.hpp`, `include/cgpui/ui/runtime_action_templates.hpp`, `tests/api_parity/typed_action_surface_test.cpp`, `tests/ui/typed_action_dispatch_test.cpp`; action macro payloads, command metadata, key routing, and bubbling still missing | Phase B action metadata/key dispatch depth |
