...
| gpui::AsyncApp | bounded prioritized async task scheduling with `try_spawn_task(...) -> Result<TaskHandle>` and `try_spawn_background_task(...) -> Result<TaskHandle>`, move-only parent/child `TaskGroup` ownership with recursive cancellation, and one-shot `AsyncIoHook` completion delivery on `WindowRuntime`, `WindowRuntimeContext`, and `AsyncContextCapability` | Required | `include/cgpui/ui/async_io_hook.hpp`, `include/cgpui/ui/task_group.hpp`, `src/ui/runtime_task_results.cpp`, `src/ui/runtime_task_pool.cpp`, `src/ui/runtime_task_priority.cpp`, `src/ui/runtime_task_group.cpp`, `src/ui/runtime_task_group_propagation.cpp`, `src/ui/runtime_async_io.cpp`, `src/ui/runtime_async_io_completion.cpp`, `tests/api_parity/async_spawn_result_conventions_test.cpp`, `tests/async/runtime_async_io_hook_test.cpp`; timer integration and cross-thread entity rules remain | Phase G |
