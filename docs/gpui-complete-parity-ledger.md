...
| gpui::TestAppContext | `TestContextCapability` groups runtime id/view id, input/event/action observability, invalidation/diagnostics snapshots with entity/window/view observer counts, timer advancement, timer cancellation, task completion draining, direct `dispatch_keystroke(...)`, grammar-backed `simulate_keystrokes(...)`, and direct pointer move/button/scroll simulation from `Context<T>::test_context()` | Adapted | `include/cgpui/ui/test_context.hpp`, `include/cgpui/ui/runtime_diagnostics.hpp`, `src/ui/test_context.cpp`, `src/ui/test_context_keystrokes.cpp`, `src/ui/test_context_pointer.cpp`, `src/ui/runtime_diagnostic_snapshot.cpp`, `tests/api_parity/test_context_capability_test.cpp`, `tests/ui/test_context_keystroke_simulation_test.cpp`, `tests/ui/test_context_pointer_simulation_test.cpp`, `tests/ui/window_runtime_scheduling_test.cpp`, `tests/ui/window_runtime_observation_diagnostics_test.cpp` | Phase G fuller simulated input/test macro depth |
...
| gpui::test | no macro-equivalent yet; public `TestContextCapability` exists with keystroke and pointer simulation | Required | internal tests plus public test-context facade, `simulate_keystrokes(...)`, and `dispatch_pointer_*` helpers, but no upstream-style test macro | Phase G |
