...
| gpui animation | deterministic animation clock/tween, typed scalar transitions, runtime-owned keyed element lifecycle animations, infinite repeat stages, indexed one-shot chains, pinned tween variants, parameterized spring curves, cancellation diagnostics, deadline-driven frame pacing, and broad production style interpolation | Required | official animation and opacity examples remain | Phase G |
