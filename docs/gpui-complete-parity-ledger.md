...
| gpui text system | `TextModel`, shaping, wrap, glyph records | Required | deterministic text depth exists; Phase D Steps 379-383 add `TextShapingBackend` selection, HarfBuzz capability reporting, explicit fallback reason metadata, glyph ids, glyph positioning offsets, shaping direction/script/language metadata, internal shaping dispatch/fallback backend boundaries, and wrapped glyph-id propagation; production HarfBuzz shaping remains incomplete | Phase D |
