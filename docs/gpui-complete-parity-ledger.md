...
| gpui text system | `TextModel`, shaping, wrap, glyph records | Required | deterministic text depth exists; Phase D Step 379 adds `TextShapingBackend` selection, HarfBuzz capability reporting, explicit fallback reason metadata, and glyph ids; production HarfBuzz shaping remains incomplete | Phase D |
