...
`Required` in the Platform Targets table denotes active target scope. In the
Required Surface Summary, it denotes an unresolved candidate gap; after Step
676 that summary has no required Windows/Linux candidate rows.
...
- X11: XCB + Vulkan strict Linux backend parity completed in Phase I.

| platform_surface | status |
|---|---|
| gpui Windows backend | Required |
| gpui_platform wayland feature | Required |
| gpui_platform x11 feature | Required |
...
depth, Android, and iOS. Required active-track work remains:
...
text and tool UI primitives, and production platform behavior across Win32,
...
| gpui::AsyncApp | bounded prioritized async task scheduling with `try_spawn_task(...) -> Result<TaskHandle>` and `try_spawn_background_task(...) -> Result<TaskHandle>`, move-only parent/child `TaskGroup` ownership with recursive cancellation, one-shot `AsyncIoHook` completion delivery, monotonic nearest-deadline runtime timers, and explicit `CrossThreadEntity<T>` FIFO read/update queueing on `WindowRuntime`, `WindowRuntimeContext`, and `AsyncContextCapability` | Adapted | `include/cgpui/ui/async_io_hook.hpp`, `include/cgpui/ui/task_group.hpp`, `include/cgpui/ui/cross_thread_entity.hpp`, `include/cgpui/platform/platform_application.hpp`, `src/ui/runtime_task_results.cpp`, `src/ui/runtime_task_pool.cpp`, `src/ui/runtime_task_priority.cpp`, `src/ui/runtime_task_group.cpp`, `src/ui/runtime_task_group_propagation.cpp`, `src/ui/runtime_async_io.cpp`, `src/ui/runtime_async_io_completion.cpp`, `src/ui/runtime_timer_integration.cpp`, `src/ui/runtime_cross_thread_entity_queue.cpp`, `src/ui/runtime_cross_thread_entity.cpp`, `src/platform/win32/win32_timer_wakeup.cpp`, `src/platform/linux/wayland_timer_wakeup.cpp`, `tests/api_parity/async_spawn_result_conventions_test.cpp`, `tests/async/runtime_async_io_hook_test.cpp`, `tests/async/runtime_async_timer_integration_test.cpp`, `tests/async/runtime_cross_thread_entity_access_test.cpp` | Phase G async runtime band closed |
...
| gpui::TestAppContext | standalone `TestApp`/`TestAppWindow` setup owns deterministic private platform and renderer state, persistent multi-window creation, stable runtime/root-view handles, typed root access, and fail-closed empty-root rejection; `TestContextCapability` groups runtime id/view id, live input/event/action observability, invalidation/diagnostics snapshots with entity/window/view observer counts, timer advancement, timer cancellation, task completion draining, run-until-parked timer/async advancement helpers, redraw/frame pump helpers including `try_draw_frame() -> Result<void>`, direct `dispatch_keystroke(...)`, grammar-backed `simulate_keystrokes(...)`, direct pointer move/button/scroll simulation, direct window activation/focus dispatch, element focus/release helpers, direct clipboard read/write helpers, and copy/cut/paste clipboard forwarding from `Context<T>::test_context()` | Adapted | `include/cgpui/ui/test_app.hpp`, `src/ui/test_app.cpp`, `src/ui/test_app_platform.cpp`, `src/ui/test_app_renderer.cpp`, `src/ui/window_runtime_internal.hpp`, `tests/ui/test_app_window_setup_test.cpp`, `tests/architecture/phase_g_test_app_window_setup_structure_test.cpp`, `include/cgpui/ui/test_context.hpp`, `include/cgpui/ui/runtime_diagnostics.hpp`, `src/ui/test_context.cpp`, `src/ui/test_context_scheduling.cpp`, `src/ui/test_context_rendering.cpp`, `src/ui/test_context_keystrokes.cpp`, `src/ui/test_context_pointer.cpp`, `src/ui/test_context_focus.cpp`, `src/ui/test_context_clipboard.cpp`, `src/ui/runtime_clipboard.cpp`, `src/ui/runtime_diagnostic_snapshot.cpp`, `examples/api_parity/public_async_test_workflow/main.cpp`, `examples/api_parity/public_phase_b_surface_closure/main.cpp`, `tests/api_parity/test_context_capability_test.cpp`, `tests/api_parity/renderer_frame_result_conventions_test.cpp`, `tests/api_parity/public_async_test_workflow_example_test.cpp`, `tests/api_parity/public_phase_b_surface_closure_example_test.cpp`, `tests/ui/test_context_time_async_test.cpp`, `tests/ui/test_context_frame_pump_test.cpp`, `tests/ui/test_context_keystroke_simulation_test.cpp`, `tests/ui/test_context_pointer_simulation_test.cpp`, `tests/ui/test_context_focus_activation_test.cpp`, `tests/ui/test_context_clipboard_test.cpp`, `tests/ui/window_runtime_scheduling_test.cpp`, `tests/ui/window_runtime_observation_diagnostics_test.cpp` | Phase G test support closeout |
...
| gpui_platform x11 feature | X11 + Vulkan | Adapted | Phase I Steps 759-798 production XCB lifecycle, Vulkan surface, input, clipboard/drag, explicit services, shared AT-SPI accessibility, diagnostics, public-example smoke, and isolated Xvfb verification | Phase I closed |
