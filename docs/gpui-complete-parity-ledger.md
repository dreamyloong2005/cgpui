...
| gpui image assets | image paint/upload pipeline, public source/registry authoring, root-confined binary loading, bounded PNG/JPEG RGBA8 decoding, and bounded animated GIF frames/delays/loop metadata | Required | SVG asset decode integration, reload/cache invalidation, async loading, and final image/GIF examples remain | Phase C/G |
