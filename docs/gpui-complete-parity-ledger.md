...
| gpui::actions! / action macro | `CGPUI_ACTIONS(...)` and `CGPUI_ACTIONS_IN(namespace_name, ...)` unit-action generation plus the existing `Action<T>` concept, stable action names, typed registration/dispatch, scoped metadata, enablement, bubbling, key grammar, keymap contexts, partial sequences, and command-palette integration | Adapted | `include/cgpui/ui/action.hpp`, `include/cgpui/ui/action_macros.hpp`, `include/cgpui/ui/key_binding.hpp`, `include/cgpui/ui/runtime_action_templates.hpp`, `include/cgpui/ui/runtime_action_enablement_templates.hpp`, `include/cgpui/ui/runtime_command_palette_templates.hpp`, `include/cgpui/ui/runtime_actions.hpp`, `src/ui/runtime_action_dispatch.cpp`, `src/ui/runtime_action_registration.cpp`, `src/ui/runtime_action_metadata.cpp`, `src/ui/runtime_command_palette_keys.cpp`, `src/ui/runtime_key_binding_grammar.cpp`, `src/ui/runtime_key_binding_modifiers.cpp`, `src/ui/runtime_key_binding_contexts.cpp`, `src/ui/runtime_key_binding_sequences.cpp`, `tests/api_parity/action_macro_surface_test.cpp`, `tests/header_cleanliness/action_macros_header_cleanliness.cpp`, `tests/architecture/phase_g_action_macro_structure_test.cpp`, and the existing typed action/key tests; payload actions remain explicit types outside the macro contract | Phase G Step 675 unit-action macro closeout |
...
| gpui accessibility | snapshot + production Win32 UIA + Linux AT-SPI D-Bus objects, navigation, roles/states, text/value/focus events, and accessibility-bus discovery/reconnect lifecycle | Required | Closed for the requested Win32/Wayland production target | Phase G |
| gpui platform services | production Win32 file open/save, directory picker, message dialog, URL, menu/chrome, reopen lifecycle, Runtime/AppContext Result policy, bounded diagnostics, and explicit Wayland/default unsupported behavior | Adapted | Phase F Step 594 platform-services closeout: `tests/architecture/phase_f_platform_services_closeout_structure_test.cpp`, the Steps 587-593 focused service guards, `src/ui/runtime_platform_service_results.cpp`, `src/ui/runtime_platform_services.cpp`, `src/ui/app_context_services.cpp`, and `src/ui/runtime_context_platform.cpp` | Phase F platform-services band closed |
| gpui Windows backend | Win32 + Vulkan | Required | active target | Phase F |
| gpui_platform wayland feature | production Wayland + Vulkan lifecycle, input, clipboard, drag/drop, services, multi-window ownership, diagnostics, packaging, examples, and real display verification | Adapted | Phase F Steps 614-618 Win32/Wayland closeout plus Phase G locked WSL Debug verification with real WSLg frame pixel capture | Windows/Linux requested target path complete pending Step 676 audit |
