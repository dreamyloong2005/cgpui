...
| gpui::AsyncApp | bounded prioritized async task scheduling with `try_spawn_task(...) -> Result<TaskHandle>` and `try_spawn_background_task(...) -> Result<TaskHandle>`, move-only parent/child `TaskGroup` ownership with recursive cancellation, one-shot `AsyncIoHook` completion delivery, and monotonic nearest-deadline runtime timers on `WindowRuntime`, `WindowRuntimeContext`, and `AsyncContextCapability` | Required | `include/cgpui/ui/async_io_hook.hpp`, `include/cgpui/ui/task_group.hpp`, `include/cgpui/platform/platform_application.hpp`, `src/ui/runtime_task_results.cpp`, `src/ui/runtime_task_pool.cpp`, `src/ui/runtime_task_priority.cpp`, `src/ui/runtime_task_group.cpp`, `src/ui/runtime_task_group_propagation.cpp`, `src/ui/runtime_async_io.cpp`, `src/ui/runtime_async_io_completion.cpp`, `src/ui/runtime_timer_integration.cpp`, `src/platform/win32/win32_timer_wakeup.cpp`, `src/platform/linux/wayland_timer_wakeup.cpp`, `tests/api_parity/async_spawn_result_conventions_test.cpp`, `tests/async/runtime_async_io_hook_test.cpp`, `tests/async/runtime_async_timer_integration_test.cpp`; cross-thread entity rules remain | Phase G |
