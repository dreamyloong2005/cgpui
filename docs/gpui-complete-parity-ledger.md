...
| gpui::AsyncApp | bounded prioritized async task scheduling plus move-only `TaskGroup` ownership, cancellation, active/total observability, and foreground/background task Result conventions on `WindowRuntime`, `WindowRuntimeContext`, and `AsyncContextCapability` | Required | `include/cgpui/ui/task_group.hpp`, `src/ui/runtime_task_pool.cpp`, `src/ui/runtime_task_priority.cpp`, `src/ui/runtime_task_group.cpp`, `src/ui/task_group.cpp`, `tests/async/runtime_task_group_test.cpp`; cancellation propagation and full async app context remain | Phase G |
