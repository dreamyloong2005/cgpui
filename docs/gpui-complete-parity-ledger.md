...
| gpui::Context<T> | `Context<T>` alias with app/window/entity helpers, `Context<T>::new_entity<T>(...)` creation, and weak entity upgrades | Adapted | `include/cgpui/ui/render.hpp`, `include/cgpui/ui/runtime_context.hpp`, `include/cgpui/ui/runtime_templates.hpp`, `tests/api_parity/context_capabilities_test.cpp`, `tests/api_parity/entity_lifecycle_creation_test.cpp`, `tests/api_parity/entity_weak_handle_semantics_test.cpp` | Phase B entity lifecycle depth |
