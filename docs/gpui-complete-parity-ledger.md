...
| gpui image assets | image paint/upload pipeline, public source/registry authoring, bounded file/image/GIF/SVG decode paths, and stable source/path/kind/revision cache keys | Required | Reload invalidation, async loading, and final image/GIF examples remain | Phase C/G |
