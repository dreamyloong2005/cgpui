...
| gpui::Context<T> | `Context<T>` alias with app/window/entity helpers and `Context<T>::new_entity<T>(...)` creation | Adapted | `include/cgpui/ui/render.hpp`, `include/cgpui/ui/runtime_context.hpp`, `include/cgpui/ui/runtime_templates.hpp`, `tests/api_parity/context_capabilities_test.cpp`, `tests/api_parity/entity_lifecycle_creation_test.cpp` | Phase B entity lifecycle depth |
...
| gpui::Entity<T> | `Entity<T>`, `Model<T>`, `EntityHandle<T>`, `WeakEntity<T>`, and context-created entity handles | Adapted | `include/cgpui/core/entity.hpp`, `include/cgpui/ui/runtime_templates.hpp`, `tests/api_parity/context_capabilities_test.cpp`, `tests/api_parity/entity_lifecycle_creation_test.cpp` | Phase B lifecycle depth |
