...
- Every retained Objective-C object has one explicit release owner.
...
- [ ] Run `xmake -r -P . phase_h_cocoa_lifecycle_structure_test macos_window_lifecycle_test && xmake test -P . phase_h_cocoa_lifecycle_structure_test/default macos_window_lifecycle_test/default`; expect missing-file and missing-behavior failures.
...
- [ ] Make `MacOSApplication` own a non-owning window registry, main-run-loop source, delayed timer, and reopen callback. The factory allocates all native objects before installing delegates and unwinds in reverse on failure.
...
- [ ] Run `xmake -r -P . phase_h_metal_bootstrap_structure_test metal_bootstrap_test && xmake test -P . phase_h_metal_bootstrap_structure_test/default metal_bootstrap_test/default`; expect the placeholder present path to fail.
...
- [ ] Run `xmake test -P . phase_h_metal_primitives_structure_test/default metal_primitive_pixel_test/default metal_text_image_pixel_test/default metal_clip_transform_pixel_test/default`; expect 4/4 pass. Commit with `git commit -m "feat: render cgpui primitives with metal"`.
...
- [ ] Run `xmake test -P . phase_h_macos_input_structure_test/default macos_input_event_test/default window_runtime_input_test/default`; expect 3/3 pass. Commit with `git commit -m "feat: add macos input translation"`.
...
- [ ] Run `xmake test -P . phase_h_macos_text_structure_test/default macos_font_discovery_test/default macos_text_input_test/default window_runtime_text_test/default`; expect 4/4 pass. Commit with `git commit -m "feat: add macos text services"`.
...
- [ ] Run `xmake test -P . phase_h_macos_clipboard_drag_structure_test/default macos_clipboard_test/default macos_drag_drop_test/default clipboard_test/default window_runtime_drag_drop_cancellation_diagnostics_test/default`; expect 5/5 pass. Commit with `git commit -m "feat: add macos clipboard and drag drop"`.
...
- [ ] Run `xmake test -P . phase_h_macos_services_structure_test/default macos_platform_services_test/default native_menu_action_dispatch_test/default`; expect 3/3 pass. Commit with `git commit -m "feat: add macos platform services"`.
...
- [ ] Run `xmake test -P . phase_h_macos_accessibility_structure_test/default macos_accessibility_test/default accessibility_pattern_runtime_test/default window_runtime_multi_window_accessibility_test/default`; expect 4/4 pass. Commit with `git commit -m "feat: add macos accessibility"`.
...
#!/usr/bin/env bash
set -euo pipefail
while IFS= read -r target; do xmake -r -P . "$target"; done < scripts/ci/macos-example-targets.txt
xmake test -P . macos_example_smoke_test/default
```
