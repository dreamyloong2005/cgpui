...
**Architecture:** Keep the active Steps 104-128 remainder intact inside the
...
- Step 104 is active in `codex/app-context-wrapper` at
  `.worktrees/app-context-wrapper`. Its baseline targeted verification passed:
...
- [ ] Steps 99-108 have landed model aliases/helpers, weak handles, observation, `AppContext`, `WindowOptions`, root-view lifecycle storage, the view registry, and child-view placeholders.
  ...
  active in `.worktrees/app-context-wrapper` with baseline targeted tests
  passing; Steps 105-108 remain.
...
For the current pre-back-40 state, continue the existing Step 104 worktree
