...
- [x] Phase G Step 673 completes Windows full-debug verification at 450/450 after updating the Phase C final ledger audit to guard stable historical SVG/image scope rather than mutable current asset status. Step 674 WSL full-debug verification is next.
...
- [x] Phase G Step 678 completes final closeout for Steps 619-678 with `tests/api_parity/phase_g_final_closeout_test.cpp`: Windows full debug passes 457/457 and WSL Arch Linux full debug passes 438/438 under Xmake 3.0.9, including real WSLg Wayland frame pixel capture and Wayland/Vulkan surface coverage; the 32-row candidate ledger remains at 0 required, 29 adapted, 2 deferred, and 1 non-goal. Phase H Step 679 Cocoa application and NSWindow lifecycle is next.
...
- [x] Steps 673-678: Run full Windows/WSL verification and declare
...
- [x] Steps 805-810: Compile every official example equivalent on every active
  ...
  - [x] macOS arm64/x86_64: compile and smoke all 20 official equivalents.
- [x] Steps 811-816: Add performance baselines: startup, first frame, resize,
  ...
  - [x] macOS arm64/x86_64: retain and validate all eight performance reports.
- [x] Steps 817-822: Add stress tests: window churn, many entities, large
  ...
  - [x] macOS arm64/x86_64: complete all eight stress scenarios.
...
- [x] Steps 829-834: Package Windows/Linux builds, CI jobs, dependency
  ...
  - [x] macOS arm64/x86_64: reproduce and verify both Release tar.gz artifacts.
- [x] Steps 835-840: Run final non-macOS matrix verification and lock the parity
  ...
  claim.
  - [x] macOS arm64/x86_64: pass Debug, examples, performance, stress, Release,
...
- Phase J Step 810 locks all 20 official examples to canonical CGPUI targets,
  ...
  execution passes in workflow run `29630949183`.
...
- Phase J Step 834 completes deterministic Windows/Linux release archives for
  ...
  macOS packaging and desktop CI pass on native arm64/x86_64 runners.
- Phase J Step 840 completes all-desktop closure with zero required gaps,
  ...
  archives. Native macOS workflow run `29630949183` passes all eight arm64 and
  x86_64 jobs at deployment target 13.0, and both Release artifacts pass full
  manifest and architecture audits. Preserved Phase H evidence stays at
  380/380. Steps 841+ remain reserved for future upstream drift.

## Immediate Next Slice

Phase J is complete for all desktop targets. Steps 841+ remain reserved for a
new pinned upstream revision and a separate parity delta plan.
